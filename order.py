...
with col2:
    ...
    funnel = Funnel()
    funnel.add(series_name = '单一环节转化率', 
...
streamlit_echarts.st_pyecharts(scatter)
...
with col2:
    ...
    bar = Bar()
    bar.add_xaxis(list(priceDict.keys())).add_yaxis("成交数", list(priceDict.values())).set_global_opts(toolbox_opts=opts.ToolboxOpts())
...
name = '订单数'
line = Line()
line.add_xaxis(xaxis_data = list(se_trans_month.index.day.map(str))).add_yaxis(series_name = name,
                                                                               y_axis = se_trans_month,).set_global_opts(yaxis_opts = opts.AxisOpts(splitline_opts = opts.SplitLineOpts(is_show = True)))
...
c.add(series_name = name,
        data_pair= [list(i) for i in se_trans_map.items()]).set_global_opts(visualmap_opts=opts.VisualMapOpts(max_=max(se_trans_map)*0.6))

c.set_series_opts(label_opts=opts.LabelOpts(is_show=False))
c.set_global_opts(
    title_opts=opts.TitleOpts(title="全国订单"),
